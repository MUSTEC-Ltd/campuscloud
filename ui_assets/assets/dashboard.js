...
function renderRecent(instances) {
  if (!instances.length) {
    renderEmpty(recentTarget, "No tracked instances are currently available.");
    return;
  }
    ...
    .map(
      (module) => `
        <a class="module-card" href="${module.ui_path}">
          <h3>${escapeHtml(module.title === "Integration Support" ? "Operations" : module.title)}</h3>
          <p>Open the operational view for this service area.</p>
          <footer>
            <span>Open view</span>
