...
async function loadInstances() {
  ...
  if (!projectId) {
    renderEmpty(listTarget, "Enter a project id to load its instances.");
    return;
  }
...
form.addEventListener("submit", async (event) => {
  ...
  try {
    ...
    setNotice(notice, "Container created.", "success");
    await loadInstances();
  } catch (error) {
...
listTarget.addEventListener("click", async (event) => {
  ...
  try {
    if (stopId) {
      ...
      setNotice(notice, "Container stopped.", "success");
    }
    if (deleteId) {
      await requestJson(`/instance/${deleteId}`, { method: "DELETE" });
      setNotice(notice, "Container deleted.", "success");
    }
    await loadInstances();
  } catch (error) {
    setNotice(notice, error.message, "error");
