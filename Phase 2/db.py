...
SCALABLE_STATUSES = ["creating", "running", "stopped"]
...
def list_scalable_instances(db_path, project_id, newest_first=False):
    sql = """
        select * from instances
        where project_id = ? and status in (?, ?, ?)
    """
    order = " order by updated_at desc" if newest_first else " order by created_at asc"
    with closing(open_db(db_path)) as conn:
        rows = conn.execute(
            sql + order,
            (project_id, *SCALABLE_STATUSES),
        ).fetchall()
    return [row_to_dict(row) for row in rows]


def count_scalable_instances(db_path, project_id):
    with closing(open_db(db_path)) as conn:
        row = conn.execute(
            """
            select count(*) as total from instances
...
def get_project_profile(db_path, project_id):
    items = list_scalable_instances(db_path, project_id, newest_first=True)
    if not items:
        return None
    item = items[0]
    return {
