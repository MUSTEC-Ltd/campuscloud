...
class UiTests(unittest.TestCase):
    ...
    def test_frontend_pages_and_assets_are_served(self):
        ...
        with TestClient(app) as client:
            ...
            for page_name, marker in [
                ("compute", "Create, scale, stop, delete, and review container instances"),
                ("network", "Inspect project-level isolation boundaries."),
                ("monitoring", "Inspect CPU, memory, and runtime usage for a project"),
                ("integration", "Review service health, quota mode, scaling status"),
            ]:
    ...
    def test_summary_and_network_views_return_expected_payloads(self):
        ...
        with TestClient(app) as client:
            ...
            self.assertEqual(summary["scalable_instance_count"], 1)
    ...
    def test_metrics_and_integration_views_return_project_state(self):
        ...
        with TestClient(app) as client:
            ...
            self.assertEqual(integration["quota"]["mode"], "allow_all")
            self.assertEqual(integration["public_apis"]["scale"], "/scale")
            self.assertEqual(integration["public_apis"]["metrics"], "/metrics/{project_id}")
            self.assertIsNotNone(integration["project_snapshot"])
            self.assertEqual(integration["project_snapshot"]["instance_count"], 1)
            self.assertEqual(integration["project_snapshot"]["scalable_instance_count"], 1)
            self.assertEqual(
                integration["project_snapshot"]["workload_profile"]["image"],
                "nginx:alpine",
            )
            self.assertEqual(integration["project_snapshot"]["billing"]["currency"], "USD")
            self.assertGreater(integration["project_snapshot"]["billing"]["amount_usd"], 0.0)
